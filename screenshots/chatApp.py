...
class ChatClient(Frame):
  ...
  def initUI(self):
    ...
    clientIPField = Entry(ipGroup, width=10, textvariable=self.clientIPVar)
    ...
    bottomLabel = Label(parentFrame, text="Created by HYH")
  ...
  def handleSendChat(self):
    if self.serverStatus == 0:
      self.setStatus("Set server address first")
      return
    name_from = self.nameVar.get().replace(' ','')
    msg = self.chatVar.get().replace(' ','')
    msg = 'from: ' + name_from + '  '+ msg
    print self.nameVar.get().replace(' ','')
    if msg == '':
        return
    self.addChat("me", msg)
